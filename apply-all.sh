#!/bin/bash

# 1. Apply Infrastructure (DBs, Brokers)
echo "Deploying Infrastructure (Postgres, Redis, Kafka)..."
kubectl apply -f infrastructure/postgres/
kubectl apply -f infrastructure/redis/
kubectl apply -f infrastructure/kafka/

echo "Waiting for infrastructure to be ready..."
sleep 15

# 2. Apply Eureka Server (Service Discovery)
echo "Deploying Eureka Server..."
kubectl apply -f eureka-server/
sleep 10

# 3. Apply Core Services
echo "Deploying Core Services (Identity, Gateway)..."
kubectl apply -f identity-service/
kubectl apply -f gateway-server/

# 4. Apply Backend Services
echo "Deploying Backend Services..."
kubectl apply -f workspace-service/
kubectl apply -f deployment-service/
kubectl apply -f infrastructure-service/

# 5. Apply Frontend
echo "Deploying Frontend..."
kubectl apply -f cloudinator-ui/

echo "✅ All services applied successfully!"
